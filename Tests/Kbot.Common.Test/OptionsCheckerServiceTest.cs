...
public class OptionsCheckerServiceTest
{
    ...
    public void CheckTestOptions_ArgumentException()
    {
        ...
        {
            Assert.ThrowsException<ArgumentException>(() => TestOptionsCheckerService.CheckPropertyForTest(value, property));
        }
    }
}

file class TestOptionsCheckerService(ILogger<TestOptionsCheckerService> logger, IOptions<TestOptions> options) : OptionsCheckerService<TestOptions>(logger, options)
{
