    ...
    public async Task<List<AggregatedOrder>> GetReportPerDayCalc(ClosedOrderQuery query){
        ...
        var grouped = orders
            ...
            .SelectMany(g => g
                .GroupBy(o => o.Type)
                .SelectMany(g => g
                    .OrderBy(o => o.CloseTimeStamp ?? DateTimeOffset.MinValue)
                    .GroupBy(o => o.CloseTimeStamp.HasValue ? o.CloseTimeStamp.Value.Date : DateTimeOffset.MinValue))
                    .Aggregate<IGrouping<DateTimeOffset, Order>, List<AggregatedOrder>>([], (list, group) => {
