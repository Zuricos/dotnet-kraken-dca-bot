...
public class HtmlService
{
  ...
  public static string GenerateDailySummary(
    List<Order> orders,
  ...
  public static string GenerateDailyTableFromOrders(List<Order> orders, MailOptions mailOptions)
  {
    ...
    sb.Append(
      "<table style='width: 100%; margin: 20px 0; border-collapse: collapse;' class='table table-striped table-bordered'>"
    );
    sb.Append("<thead><tr style='background-color: #f2f2f2; color: #333;'>");
    sb.Append(
      "<th style='padding: 12px; text-align: center; border: 1px solid #ddd;'>Order ID</th>"
    );
    sb.Append(
      "<th style='padding: 12px; text-align: center; border: 1px solid #ddd;'>Executed Time UTC</th>"
    );
    sb.Append("<th style='padding: 12px; text-align: center; border: 1px solid #ddd;'>Price</th>");
    if (mailOptions.Crypto.Equals("BTC"))
      sb.Append(
        $"<th style='padding: 12px; text-align: center; border: 1px solid #ddd;'>Amount (Sats)</th>"
      );
    else
      sb.Append(
        $"<th style='padding: 12px; text-align: center; border: 1px solid #ddd;'>Amount ({mailOptions.Crypto})</th>"
      );
    sb.Append(
      $"<th style='padding: 12px; text-align: center; border: 1px solid #ddd;'>Cost ({mailOptions.Fiat})</th>"
    );
    sb.Append(
      $"<th style='padding: 12px; text-align: center; border: 1px solid #ddd;'>Fee ({mailOptions.Fiat})</th>"
    );
    ...
    for (int i = 0; i < orders.Count; i++)
    {
      ...
      sb.Append(
        $"<td style='padding: 12px; text-align: center; border: 1px solid #ddd;'>{order.OrderId}</td>"
      );
      sb.Append(
        $"<td style='padding: 12px; text-align: center; border: 1px solid #ddd;'>{order.CloseTimeStamp?.ToString("u", CultureInfo.InvariantCulture)}</td>"
      );
      sb.Append(
        $"<td style='padding: 12px; text-align: center; border: 1px solid #ddd;'>{order.Price:F2}</td>"
      );
      if (mailOptions.Crypto.Equals("BTC"))
        sb.Append(
          $"<td style='padding: 12px; text-align: center; border: 1px solid #ddd;'>{order.Volume * SATS:N0}</td>"
        );
      else
        sb.Append(
          $"<td style='padding: 12px; text-align: center; border: 1px solid #ddd;'>{order.Volume:F6}</td>"
        );
      sb.Append(
        $"<td style='padding: 12px; text-align: center; border: 1px solid #ddd;'>{order.Cost:F2}</td>"
      );
      sb.Append(
        $"<td style='padding: 12px; text-align: center; border: 1px solid #ddd;'>{order.Fee:F2}</td>"
      );
      sb.Append("</tr>");
    }
  ...
  public static string GenerateReportWelcome(DateTimeOffset start, DateTimeOffset end)
  {
    return $"""
          <div style='text-align: center; font-size: 16px; margin-top: 30px;'>
              ...
              The report contains all closed orders from {start.ToString(
        "u",
        CultureInfo.InvariantCulture
      )} to {end.ToString("u", CultureInfo.InvariantCulture)}. <br/><br/>
          </div>";
      """;
  }
}
