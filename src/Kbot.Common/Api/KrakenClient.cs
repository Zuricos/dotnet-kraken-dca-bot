...
public sealed class KrakenClient(ILogger<KrakenClient> logger, KrakenApi api) : IDisposable
{
    ...
    public async Task<Dictionary<string, double>> CheckBalance()
    {
        try
        {
            var response = await api.PostPrivateAsync<Dictionary<string, string>>(PrivateMethod.Balance, new Dictionary<string, object>());
            if (HasError(response)) return [];

            logger.LogInformation("Current balance: {Balance}", string.Join(", ", response.Result!.Select(kvp => $"{kvp.Key}: {kvp.Value}")));
    ...
    public async Task<double> GetCurrentCryptoPrice(string pair = "XXBTZUSD")
    {
        try
        {
            var response = await api.GetPublicAsync<Dictionary<string, TickerInfoUnparsed>>(
                       PublicMethod.Ticker, new Dictionary<string, string> { { "pair", pair } });
            if (HasError(response)) return 0.0;
            TickerInfo tickerInfo;
            tickerInfo = response.Result![pair].Parse();

            logger.LogInformation("Current pair {Pair} price is: {CurrentPrice}", pair, tickerInfo.Ask.Price);
    ...
    public async Task<bool> SendOrder(OrderRequest orderData)
    {
        ...
        try
        {
            response = await api.PostPrivateAsync<OrderResponse>(PrivateMethod.AddOrder, orderData.ToDictionary());
        ...
        logger.LogInformation("Order sent: {Order} for {Descr}", response.Result!.TransactionIds, response.Result!.Description.Order);
        return true;
    }
    ...
    public async Task<bool> CancelOrder(string? cl_ord_id, string? orderId)
    {
        ...
        try
        {
            var response = await api.PostPrivateAsync<CancelOrderResponse>(PrivateMethod.CancelOrder, orderData);
            if (HasError(response)) return false;
    ...
    public async Task<ClosedOrders?> GetClosedOrders(bool fetchAll = true, int offset = 0, DateTimeOffset start = default, DateTimeOffset end = default)
    {
        var orderData = new Dictionary<string, object>
        {
            { "ofs", offset }
        ...
        try
        {
            response = await api.PostPrivateAsync<ClosedOrderUnparsed>(PrivateMethod.ClosedOrders, orderData);
        ...
        try
        {
            ...
            logger.LogInformation("In the given time frame are {Count} closed orders", closedOrders.Closed.Count);
