...
public class HolidayService(ILogger<HolidayService> logger, IOptions<CultureOptions> cultureInfo)
  : IHostedService
{
  public ConcurrentDictionary<int, List<PublicHoliday>> Holidays { get; set; } = [];
  public string CountryCode =>
    new RegionInfo(cultureInfo.Value.CultureInfo.Name).TwoLetterISORegionName;
  ...
  private async Task FetchHolidaysFromRemote(
    int year,
  ...
  )
  {
    try
    {
      ...
      if (!response.IsSuccessStatusCode)
        throw new Exception("Failed to fetch holidays for year " + year);
      ...
      var publicHolidays =
        ...
        ?? throw new Exception("Failed to deserialize holidays for year " + year);

      var filteredPublicHolidays = publicHolidays
        .Where(h =>
          (h.Counties == null || h.Counties.Contains(CountyCode)) && h.Types.Contains("Public")
        )
        .ToList();
